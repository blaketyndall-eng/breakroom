...
Create a new Supabase project for The Breakroom.
...
Open Supabase Dashboard > SQL Editor and paste/run each file in order.

### Option B — Supabase CLI

Install/login/link with Supabase CLI, then push migrations:
...
5. `/after-hours` blocks before Clock Out.
6. `/clock-out` completes.
7. Supabase `user_profiles.shift_status` changes to `clocked_out`.
8. `/after-hours` allows entry after Clock Out.
9. Refresh still preserves access.
10. Sign out / new browser should require login again.
